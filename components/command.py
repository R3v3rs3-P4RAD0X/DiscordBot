...
class Command:
    ...
    help: Help = Help
    permissions: Permissions = Permissions(False, "send_messages", "read_messages")
    ...
    def __init__(self, message: discord.Message, args: list[str], client: any = None, config: dict = {}):
        ...
        self.client = client
        self.config = config
        self.user_permissions = Permissions(self.message.author.guild_permissions.value)
        self.bot_permissions = Permissions(self.message.guild.me.guild_permissions.value)
        self.ratelimit = Ratelimit(self, 1, 5, type="command")
    ...
    async def check(self) -> (bool, str):
        ...
        if self.bot_permissions < self.permissions:
            # Get the permissions that the bot doesn't have
            missing = self.permissions.missing(self.bot_permissions)
            ...
            await self.send(f"I don't have the required permissions to run this command. Missing permissions: {', '.join(missing)}")
            return True, "I don't have the required permissions to run this command."
        
        # Check if the user has the required permissions
        if self.user_permissions < self.permissions:
            # Get the permissions that the user doesn't have
            missing = self.permissions.missing(self.user_permissions)

            # Send the missing permissions message
            await self.send(f"You don't have the required permissions to run this command. Missing permissions: {', '.join(missing)}")
            return True, "You don't have the required permissions to run this command."
        
        # Check if the command is ratelimited
        if self.ratelimit.limited(self.message.guild.id if self.ratelimit.type == "guild" or isinstance(self.ratelimit, tuple) else self.message.author.id):
